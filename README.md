...
- **Google Cloud Platform:** Framework utilized for orchestrating complex agentic workflows and tool execution.
...
1. **OrchestratorAgent:** The central routing hub. It receives user messages, determines their intent using a specialized Gemini prompt, and delegates the task to the appropriate sub-agent.
...
3. **CodeExplainerAgent:** The domain expert. When a student is stuck or asks about a specific file/concept, this agent researches the repository, explains the code in an educational manner, and delegates updates to the Concept Map.
4. **ProfilerAgent:** The passive observer. It analyzes the chat history and the Explainer's outputs to update the student's `Concept Map` in MongoDB.
...
Create a `.env` file in the root directory (for the backend) and another in the `frontend/` directory with the following configuration:
...
# Databases
MONGO_URI=mongodb://localhost:27017/gitlearn
